	...
	fn copy_to_rgba_pixels(&self, pixels: &mut [u8]) {
		for y in 0..SCREEN_HEIGHT {
			for x in 0..SCREEN_WIDTH {
				...
				// Internal storage is BGR, output must be RGBA
				pixels[base_index * 4 + 0] = self.pixels[base_index * 3 + 2]; // R
				pixels[base_index * 4 + 1] = self.pixels[base_index * 3 + 1]; // G
				pixels[base_index * 4 + 2] = self.pixels[base_index * 3 + 0]; // B
				pixels[base_index * 4 + 3] = 255;                              // A
			}
		}
	}
}
